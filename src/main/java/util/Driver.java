	...
	private static DesiredCapabilities getCapabilities() {
		return new DesiredCapabilities(
				new HashMap<>() {{
					...
					put("platformName", device.getPlatformName());
					// Версия ОС
					put("platformVersion", device.getPlatformVersion());
					// Уникальный идентификатор подключенного устройства
					put("udid", device.getUdid());
					// Java-пакет Android приложения, которое мы хотим запустить. (APK info)
					put("appPackage", "net.metaquotes.economiccalendar");
					// Имя activity которые мы хотим запустить из пакета, указанного выше. (APK info)
					put("appActivity", "net.metaquotes.ui.MainActivity");
				}}
		);
	}
